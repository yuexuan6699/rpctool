...
// ==================== Aria2 Client ====================
const Aria2Client = {
  async sendDownload(rpcPath, fileDownloadInfo) {
    try {
      console.log('[Aria2Client] sendDownload called with:', { rpcPath, fileDownloadInfo });
      
      const { authStr, path, options } = URLUtils.parseRPCUrl(rpcPath);
  ...
  handleClick(info) {
    ...
    if (info.menuItemId === 'rpctool-single') {
      ConfigManager.getRpcList().then(rpcList => {
        if (rpcList.length > 0) {
          Aria2Client.sendDownload(rpcList[0].path, { link: info.linkUrl });
        }
      });
      return;
    }
    ...
    if (typeof info.menuItemId === 'string' && info.menuItemId.startsWith('rpctool-rpc-')) {
      ...
      ConfigManager.getRpcList().then(rpcList => {
        ...
        if (index >= 0 && index < uniqueRpcList.length) {
          Aria2Client.sendDownload(uniqueRpcList[index].path, { link: info.linkUrl });
        }
      });
    }
  }
